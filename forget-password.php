...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['cust_email'])) {
        ...
        if (filter_var($_POST['cust_email'], FILTER_VALIDATE_EMAIL) === false) {
            ...
            if($total == 0) {
                $valid = 0;
                $error_message .= "L'email n'est pas reconnu.\\n";
            }
        }
    }
    ...
    if($valid == 1) {
        ...
        $statement->execute(array($token, $now, $_POST['cust_email']));
        ...
        try {
            ...
            $mail->SMTPAuth   = true;
            $mail->Username   = 'youneschell2000@gmail.com'; // Remplacez par votre adresse email
            $mail->Password   = 'cacmbvrvcuxmkeen'; // Remplacez par votre mot de passe
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // 'ssl' également possible selon votre configuration SMTP
            $mail->Port       = 587; // Utilisez 465 pour 'ssl'

            // Expéditeur et destinataire
             //$mail->setFrom('youneschell2000@gmail.com', 'Votre Site'); Remplacez par votre adresse email
            $mail->addAddress($_POST['cust_email']); // Destinataire

            // Contenu de l'email
            $mail->isHTML(true);
            $mail->Subject = 'Réinitialisation de votre mot de passe';
            $mail->Body    = '<p>'.LANG_VALUE_142.'<br> <a href="'.BASE_URL.'http://localhost/ecommerce-larbi/reset-password.php?email='.$_POST['cust_email'].'&token='.$token.'">Click ici</a>';

            $mail->send();
            $success_message = $forget_password_message;
        } catch (Exception $e) {
            $error_message .= "L'email n'a pas pu être envoyé. Veuillez réessayer plus tard. Erreur: {$mail->ErrorInfo}\\n";
        }
    }
}
?>
